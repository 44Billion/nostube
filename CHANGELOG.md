...
- **Video Player Resume Position Not Working**: Fixed resume from last play position (stored in localStorage) and `?t=` URL parameter not seeking to the correct time. The initial position was being set before the video had loaded its metadata, so setting `currentTime` had no effect. Now waits for `loadedmetadata` event before seeking (VideoPlayer.tsx:216-238)
