...
- **App Presets System**: NIP-78 based preset system (kind 30078) for app-wide configuration. Admins can manage blocked pubkeys, NSFW authors, default relays, blossom proxy, and thumbnail resize server via `/admin` route. Users can browse and select presets from other users via `/presets` route. Selected preset stored in localStorage, with 1-hour cache for performance. Default preset: `npub1klr0dy2ul2dx9llk58czvpx73rprcmrvd5dc7ck8esg8f8es06qs427gxc`.
