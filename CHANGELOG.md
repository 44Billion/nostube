...
- **Unified Draft & Upload Manager**: UploadManagerProvider now owns both task state and draft state as a single source of truth. When DVM transcoding completes, videos are added directly to the draft's `uploadInfo.videos` in the provider, eliminating race conditions from stale callbacks. Draft CRUD operations (create, update, delete) are now part of the manager context. Nostr sync is debounced for form fields (5s) and immediate for upload milestones. Pure storage functions extracted to `draft-storage.ts` (UploadManagerProvider.tsx, draft-storage.ts)
- **Global Upload Manager with Background Transcoding**: Upload manager now runs DVM transcode subscriptions in global state, allowing users to navigate away while transcoding continues. Created `useDvmTranscodeManager` hook as thin wrapper for components. Provider owns all Nostr subscriptions and auto-resumes incomplete tasks on app start. Unified notification system shows both video comment and upload notifications in the same bell dropdown (UploadManagerProvider.tsx, useDvmTranscodeManager.ts, upload-manager-storage.ts, upload-manager.ts types, useUploadNotifications.ts, useAllNotifications.ts, UploadNotificationItem.tsx)
...
- **DVM Transcoded Videos Not Added to Draft**: Fixed race condition where transcoded videos weren't added to the draft's video list when the upload dialog was closed during transcoding. The `onComplete` callback became stale when the dialog unmounted. Now the UploadManagerProvider directly updates the draft's `uploadInfo.videos` when each transcode resolution completes, bypassing callbacks entirely. Also includes duplicate URL check to prevent the same video being added twice (UploadManagerProvider.tsx)
...
- **DVM Transcode Job Not Found Race Condition**: Fixed "Job not found" error immediately when starting DVM transcode. The auto-resume effect was triggering during `startTranscode` because updating task state caused `resumeTranscode` callback to change, re-running the effect. Added `startingTasksRef` to track tasks being started and guards in both auto-resume effect and `resumeTranscode` to skip tasks that already have active jobs (UploadManagerProvider.tsx:122-123, 724-725, 812-814, 827-833, 1071-1083)
- **DVM Transcode Queue Undefined Access**: Fixed "Cannot read properties of undefined (reading '0')" error in DvmTranscodeAlert when queue data was partially undefined. Added proper optional chaining for all queue property accesses: `queue?.resolutions?.[index]`, `queue?.completed?.includes()`, `queue?.currentIndex ?? 0` (DvmTranscodeAlert.tsx:252-254, 385-396, 406)
- **Draft Deletion Not Removing Notifications**: Deleting a draft now also removes related upload/transcode notifications. Added `removeByDraftId` function to `useUploadNotifications` and called it in both delete handlers in `DraftPicker` (useUploadNotifications.ts:177-186, DraftPicker.tsx:44, 73)
- **Duplicate Upload Notifications**: Fixed duplicate notification key warnings in React. Added ID deduplication in `cleanupOldNotifications` and duplicate prevention for same type/draftId within 10 seconds in `addNotification` (useUploadNotifications.ts:48-54, 114-124)
- **Transcoded Videos Not Added to Draft on Remount**: Fixed transcoded videos not being added to the draft when reopening the upload dialog after transcoding completed in the background. The callbacks stored in the job became stale when the dialog was closed. Now stores completed videos in `transcodeState.completedVideos` and delivers them via `onComplete` when the component remounts (UploadManagerProvider.tsx:784-813, useDvmTranscodeManager.ts:143-193, upload-manager.ts:TranscodeState.completedVideos)
