...
- **Cashu Mint Dropdown**: The "Add Mint" input field now shows a dropdown with popular Cashu mints (minibits.cash, coinos.io, lnbits.com) instead of a plain text input. Already-added mints are filtered out. Custom URLs can still be entered via the search input.
...
- **Cashu Wallet Support (NIP-60)**: Self-custodial e-cash wallet support using `applesauce-wallet`. Create new Cashu wallets stored encrypted on Nostr relays, or unlock existing wallets. Pay Lightning invoices by melting Cashu tokens. Supports multiple mints with balance display per mint. Wallet Settings UI includes tabs for switching between NWC (Lightning) and Cashu wallet types.
...
- **Cashu Mint Dropdown Selection**: Fixed clicking on popular mints in the "Add Mint" dropdown not working. The cmdk Command component's built-in filtering was interfering with click selection when using a controlled CommandInput. Added `shouldFilter={false}` to disable built-in filtering since mints are already manually filtered. Also added `onMouseDown` handler with `preventDefault` to ensure clicks register before focus blur events can interfere with selection.
- **Cashu Unlock Button Visibility**: The "Unlock Wallet" button now only appears when the Cashu wallet is actually locked. Previously it showed even after the wallet was already unlocked. Added `isCashuWalletUnlocked` state to WalletContext using `WalletHelpers.isWalletUnlocked()`.
