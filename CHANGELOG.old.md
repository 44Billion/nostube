...
- DVM transcoding: display active DVM name and description above progress status during transcoding, mirroring, and resume states; DVM info is resolved from the background tracker's handler map using the active DVM pubkey; new `DvmInfoBanner` component with Bot icon; `dvmPubkey` surfaced through `TranscodeProgress` type
...
- Playlists: use `a` tags for addressable video events (kinds 34235/34236) instead of `e` tags; when a video is edited its address stays stable so playlists no longer break; `e` tags still used for regular events (kinds 21/22); Video interface gains optional `address` field; `usePlaylistDetails` resolves `a`-tag refs via `eventStore.replaceable()` and `addressLoader`; duplicate detection in `AddToPlaylistButton` checks both event ID and address; drag-and-drop reorder preserves address; modified `usePlaylist.ts`, `usePlaylistDetails.ts`, `AddToPlaylistButton.tsx`, `VideoInfoSection.tsx`, `SinglePlaylistPage.tsx`
- DVM transcoding: replaced on-demand DVM availability check with background tracker that monitors recent NIP-89 handler announcements (kind 31990) from the last 10 minutes; DVMs are stored in a hashmap by pubkey with name/about metadata for future UI display; tracker uses module-level singleton pattern with `useSyncExternalStore` for zero-overhead React integration; stale entries pruned every 60 seconds; `startTranscode` now uses tracked DVMs for instant DVM selection (skips bidding phase); deleted `useDvmAvailability.ts` in favor of new `useDvmTracker.ts`; added `getTrackedDvms()` non-hook getter for use in `UploadManagerProvider`
...
- Comments: fixed kind 1 (text note) replies not threading into a tree; `mapEventToComment` always used the first `e` tag as the parent reference, but kind 1 events use NIP-10 markers (`root`/`reply` in position [3]) where the first `e` tag is typically the `root` (video) not the `reply` (parent comment); now parses NIP-10 markers for kind 1 events: `reply` marker identifies the parent comment, `root` marker identifies the video; also handles deprecated positional scheme (no markers) where last `e` tag = reply
- Video player: fixed playback glitch ~5 seconds after initial load where video would briefly stop/start; stall detection effect re-ran when URL discovery or config loading changed its dependencies (`moveToNextVideo`, `hasMoreVideoUrls`), resetting the 5-second timeout even though the video was already playing; `loadedmetadata`/`canplay` events don't re-fire for the same source so the timeout would fire and switch URLs unnecessarily; fixed by: (1) moving `moveToNextVideo` and `hasMoreVideoUrls` into refs so the effect only re-runs when `videoUrl` actually changes, (2) skipping stall detection if video is already playing (`readyState >= 3`), (3) adding `playing` event listener for additional cancellation, (4) checking playback state at timeout fire time as a safety net; also fixed URL regeneration in `useMediaUrls` to merge new URLs without resetting `currentIndex` when config changes after playback has started
